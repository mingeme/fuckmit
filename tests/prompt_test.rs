...
fn test_prompt_init_global() {
    ...
    assert.success()
        .stdout(predicate::str::contains("Created commit configuration"));
...
fn test_prompt_list() {
    ...
    assert.success()
        .stdout(predicate::str::contains("Available prompt configurations:"))
...
fn test_prompt_use() {
    ...
    assert.success()
        .stdout(predicate::str::contains("Now using 'custom' as the active prompt configuration"));
    ...
    // Run the prompt list command to verify that custom is marked as active
    let mut cmd = Command::cargo_bin("fuckmit").unwrap();
    let assert = cmd
        .env("FUCKMIT_CONFIG_DIR", config_dir)
        .arg("prompt")
...
#[test]
fn test_prompt_use_nonexistent() {
    let temp_dir = setup_temp_config_dir();
    let config_dir = temp_dir.path().to_str().unwrap();
    
    // Run the prompt use command with a non-existent configuration
    let mut cmd = Command::cargo_bin("fuckmit").unwrap();
    let assert = cmd
        .env("FUCKMIT_CONFIG_DIR", config_dir)
        .arg("prompt")
...
fn test_prompt_show() {
    ...
    // Run the prompt show command (should use the global default config)
    let mut cmd = Command::cargo_bin("fuckmit").unwrap();
    let assert = cmd
        .env("FUCKMIT_CONFIG_DIR", config_dir)
        .arg("prompt")
    ...
    assert.success()
        .stdout(predicate::str::contains("Current commit configuration:"))
        .stdout(predicate::str::contains("You are a helpful assistant"));
}

#[test]
fn test_prompt_show_local() {
    ...
    // Run the prompt show command (should use the local config)
    let mut cmd = Command::cargo_bin("fuckmit").unwrap();
    let assert = cmd
        .env("FUCKMIT_CONFIG_DIR", config_dir)
        .arg("prompt")
